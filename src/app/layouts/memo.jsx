      ...
      <>
         ...
         <CollapseWrapper>
            <BlockQuoteWrapper>
               <p>
                  ...
                  случаях, мемоизируя тем самым результат. Это значит, что React
                  будет использовать результат последнего рендера, избегая
         ...
         <CollapseWrapper title="Когда использовать">
            ...
            <ul>
               ...
               <li>
                  Компоненту передаются одинаковые параметры при нескольких
                  ререндерах
               </li>
         ...
         <CardWrapper>
            <i className="bi bi-lightbulb"></i> Если функциональный компонент
            обёрнут в <code>React.memo</code> и использует{" "}
            <code>useState, useReducer или useContext</code>, он будет повторно
            рендериться при изменении состояния или контекста.
         </CardWrapper>
         <CardWrapper>
         ...
         <CardWrapper>
            ...
            <CardWrapper>
               <i className="bi bi-lightbulb"></i> Если функция для сверки не
               задана, то выполняется поверхностная сверка по{" "}
               <Link
                  to={{
                     pathname:
